...
func Load(projectPath string) (string, error) {
	...
	if s := string(b); s == seedV1 || s == seedV2 || s == seedV3 || s == seedV4 || s == seedV5 || s == seedV6 || s == seedV7 {
		if err := writeAtomic(p, []byte(Seed)); err != nil {
...
// versions are kept below (seedV1 through seedV7) so Load can recognize an
...
## Ship policy
dispatch with risk=ship, pipeline_run auto, and pipeline_run ship all return
a confirmation token. Relay the token to the user VERBATIM and resubmit only
after the user types it back. Never invent, guess, or self-supply a token.

## Shipping commits / PRs
Never hand-roll git push or gh pr create to publish committed work — route
publication through pipeline_run ship instead. It builds a validated PR: a
context packet from git facts, truth-rendered test/review status, prose
validated against the diff, a static fallback when validation fails, and
centralized attribution. It is confirm-token gated, same as auto. A
hand-rolled PR body claiming unvalidated things like "tests pass" is exactly
what this pathway exists to prevent.
`

// seedV7 is the pre-ship-pathway conductor seed (shipped before the styx
// ship verb / pipeline_run ship). Kept verbatim so Load can detect an
// unmodified v7 file and upgrade it transparently.
const seedV7 = `# Styx conductor guidance
